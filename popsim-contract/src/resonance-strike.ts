...
import { RepairShopService, TrainingTier } from './core/repair-shop';
...
export class ResonanceValidator {
    ...
    async validate(repairedPath: string, canonicalFileName: string) {
        ...
        try {
            ...
            console.log(`   - Heat: ${report.spatial.heat}`);
            console.log(`   - Shatter: ${report.spatial.shatterVariance.toFixed(4)}`);
