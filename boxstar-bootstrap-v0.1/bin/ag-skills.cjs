...
const os = require('os');
const { listSkillIds, readSkill, tokenize, unique } = require('../lib/skill-utils.cjs');
let version = '0.1.0-bootstrap';
try {
  const pkg = require('../../package.json');
  version = pkg.version;
} catch (e) {
  // Use fallback if package.json is missing in dist
}
...
function _readSkillInternal(sourceDir, skillId) {
  const skillPath = path.join(sourceDir, skillId);
  const manifestPath = path.join(skillPath, 'manifest.json');
  const packedPath = path.join(skillPath, 'instructions.packed');
  
  let manifest = { id: skillId, name: skillId, description: '' };
  ...
  if (fs.existsSync(packedPath)) {
    ...
    manifest._instructions = decodePacked(packed);
  }
...
function loadCatalog() {
  ...
  const skills = skillIds.map(skillId => {
    const skill = _readSkillInternal(SKILLS_SOURCE_DIR, skillId);
