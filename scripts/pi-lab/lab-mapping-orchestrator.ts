...
// ─────────────────────────────────────────
// CHUNK + EMBED — mean-pool back to 1024-D
// Preserves spectral density of full shard
// ─────────────────────────────────────────
async function embed(text: string, chunkSize = 500, overlap = 50): Promise<number[]> {
  const chunks: string[] = [];
  ...
  // Mean-pool across chunks → single 1024-D vector
