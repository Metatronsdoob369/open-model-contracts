...
-- CANONICAL BRIDGE INJECTION: Orchestrated_Repair_Bridge
...
local Orchestrated_Repair_Bridge_LastFire = 0
local Orchestrated_Repair_Bridge_ThrottleRate = 15

local function SafeFireOrchestrated_Repair_Bridge(payload)
    local now = os.clock()
    if now - Orchestrated_Repair_Bridge_LastFire < (1 / Orchestrated_Repair_Bridge_ThrottleRate) then
        return
    end

    Orchestrated_Repair_Bridge_LastFire = now

    Orchestrated_Repair_Bridge:FireServer(payload)
end

-- RECEIVER-SIDE GUARD
Orchestrated_Repair_Bridge.OnServerEvent:Connect(function(player, payload)
    local now = os.clock()
    if not Orchestrated_Repair_Bridge_LastReceive then Orchestrated_Repair_Bridge_LastReceive = 0 end
    if now - Orchestrated_Repair_Bridge_LastReceive < 0.05 then
        return
    end
    Orchestrated_Repair_Bridge_LastReceive = now
