import { z } from 'zod';
import { AERIAL_HAWK_PRESET, validateEnemyArchetype } from './enemy-archetype-contract.js';

// Re-export so consumers can import from director-contract without knowing the split
export { AERIAL_HAWK_PRESET, validateEnemyArchetype };
...
export const DirectorOutputSchema = z.object({
  ...
  // Archetype activation — which typed enemy presets are included in this directive.
  // Prevents hallucination: specialists get a schema-validated archetype, not NL.
  activatedArchetypes: z.array(z.object({
    archetypeId:      z.string(),
    archetypeVersion: z.string(),
    weppyValidated:   z.boolean().describe('True = Weppy MCP confirmed asset IDs exist in Studio'),
  })).default([]),
});
