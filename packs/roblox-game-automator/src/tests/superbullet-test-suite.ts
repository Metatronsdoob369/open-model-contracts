...
const TEST_CONFIG = {
  ...
  embedUrl: process.env.EMBED_URL, // a-mem-3072-refrag endpoint
  qdrantUrl: process.env.QDRANT_URL, // base URL, e.g., http://localhost:6333
  qdrantCollection: process.env.QDRANT_COLLECTION || 'refrag-3072',
  brokenLuaPath:
    process.env.BROKEN_LUA ||
    '/Users/joewales/NODE_OUT_Master/open-model-contracts/workspace/gen/job2-stubs/full/pizza-superbullet.lua',
};

const HAS_EXTERNAL_BROKEN =
  fs.existsSync(TEST_CONFIG.brokenLuaPath) &&
  fs.statSync(TEST_CONFIG.brokenLuaPath).size > 0;
...
const BROKEN_VARIANTS = HAS_EXTERNAL_BROKEN
  ? [
      {
        name: 'natural-stub',
        fault: `External broken file: ${TEST_CONFIG.brokenLuaPath}`,
        transform: () => fs.readFileSync(TEST_CONFIG.brokenLuaPath, 'utf-8'),
      },
      ...VARIANTS_BASE,
    ]
  : VARIANTS_BASE;
...
class SuperbulletTestSuite {
  ...
  async runBaselineRepair(): Promise<TestResult[]> {
    ...
    for (const variant of BROKEN_VARIANTS) {
      ...
      try {
        ...
        console.log(`   ✅ Result: ${result.success ? 'FIXED' : 'FAILED'} (${duration}ms) [Provider: ${result.provider}]`);
  ...
  private async generateEmbedding(code: string): Promise<number[]> {
    if (!TEST_CONFIG.embedUrl) {
      // Fallback mock if no embedder configured
      return Array(3072).fill(0).map(() => Math.random() * 2 - 1);
    }

    const resp = await fetch(TEST_CONFIG.embedUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!resp.ok) {
      throw new Error(`Embedder HTTP ${resp.status}`);
    }
  ...
  private async querySpatialMap(embedding: number[]): Promise<any> {
    if (!TEST_CONFIG.qdrantUrl) {
      // Mock fallback
      return {
        queryPoint: embedding.slice(0, 3),
        neighbors: [
    ...
    const url = `${TEST_CONFIG.qdrantUrl}/collections/${TEST_CONFIG.qdrantCollection}/points/search`;
    ...
    if (!resp.ok) {
      throw new Error(`Qdrant HTTP ${resp.status}`);
    }

    const data = await resp.json() as { result?: Array<{ id: string | number; score: number; payload?: any }> };
    const neighbors = (data.result || []).map(n => ({
      id: typeof n.id === 'string' ? n.id : String(n.id),
      distance: n.score,
      quality: n.payload?.quality ?? 'unknown',
    }));
    ...
    return {
      ...
      heatMap: {
        ...
        y: embedding[1],
        z: embedding[2],
        intensity: neighbors.length ? Math.max(0, 1 - neighbors[0].distance) : 0,
      },
    };
  }
