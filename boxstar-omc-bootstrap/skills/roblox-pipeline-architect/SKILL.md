...
### 2. Constraint System Design & Shatter Analysis
...
2. **Apply Shatter Velocity Math**: 
   - Formula: `V_shatter = (D_spatial / Δ_raw) * (1 / T_expected) * W_phase`
   - If `V_shatter > 1.0`, the fracture is catastrophic. You MUST prioritize repair over new features.
   - If `V_shatter < 0.2`, the component is **Diamond-Stable**.
3. Refer to the `structural_coherence.json` library for CFrame/Part rules.
4. Design a deterministic repair pipeline using the Swarm agents (Surveyor, Architect, Landscaper, Inspector).
5. Define the Qdrant feedback loop where successful repairs become future templates.
